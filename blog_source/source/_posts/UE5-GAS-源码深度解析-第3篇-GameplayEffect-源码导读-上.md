...
`FGameplayModifierInfo` 是 GE 里配置 Modifier 的结构，核心字段就三个：

- `Attribute`：要改哪个属性（`FGameplayAttribute` 句柄）
- `ModifierOp`：怎么改（`EGameplayModOp` 枚举）
- `Magnitude`：改多少（`FGameplayModifierMagnitude`）

`ModifierOp` 的四种运算语义：

- `Add`：加法叠加，`Current = Current + Magnitude`
- `Multiply`：乘法放大，`Current = Current * Magnitude`
- `Divide`：除法缩小，`Current = Current / Magnitude`
- `Override`：直接覆盖，`Current = Magnitude`（慎用，会覆盖前面所有 Modifier 的贡献）

概念本身不玄，重点在 `Magnitude` 怎么算——这是 Modifier 的核心。

### Magnitude 的三种计算方式

`FGameplayModifierMagnitude` 支持三种计算方式，源码里用 `EModifierMagnitudeType` 枚举区分：

```cpp
// GameplayEffectModifierMagnitude.h
enum class EModifierMagnitudeType : uint8
{
    Scalable,        // 可缩放的数值（CurveTable 查表）
...
计算逻辑在 `FScalableFloat::GetValueAtLevel`：

```cpp
// GameplayEffectModifierMagnitude.cpp
float FScalableFloat::GetValueAtLevel(const float Level) const
{
    if (CurveTable && !CurveRowName.IsNone())
    {
        // 从 CurveTable 查表
        const FSimpleCurve* Curve = CurveTable->FindSimpleCurve(CurveRowName);
        if (Curve)
        {
            return Curve->Eval(Level);
        }
    }
    return Value;  // 没配表就直接返回 Value
}
```

**设计意图**：让策划配一张表，不同 Level 对应不同数值。比如"技能等级 1 → +10 攻击力，等级 2 → +20 攻击力"，GE 施加时传入 Level，自动查表得到数值。

**调用时机**：Spec 创建时调用，得到 `EvaluatedMagnitude`（已计算的数值），后续运算直接用这个值，不再重新查表。

#### Attribute Based：从另一个属性取值

从 Source 或 Target 的 ASC 上捕获另一个属性的值，再乘以系数。`FAttributeBasedFloat` 的源码：
...
struct FAttributeBasedFloat
{
    ...
    UPROPERTY()
    FGameplayEffectAttributeCaptureSpec AttributeToCapture;

    UPROPERTY()
    float Coefficient = 1.f;

    UPROPERTY()
    EGameplayEffectAttributeCaptureSource Source = EGameplayEffectAttributeCaptureSource::Source;
};
```

计算逻辑在 `FAttributeBasedFloat::Calculate`：
...
- `DurationPolicy`：`EGameplayEffectDurationPolicy`，Instant / Duration / Infinite（下一节深挖）
- `DurationMagnitude`：`FScalableFloat`，持续时间（秒）
...
- `Executions`：`TArray<FGameplayEffectExecutionInfo>`，ExecutionCalculation（第 4 篇）

概念本身不玄，重点是 `DurationPolicy`——它决定了 GE 的执行语义和生命周期。

### DurationPolicy：Instant / Duration / Infinite 的实现差异

`EGameplayEffectDurationPolicy` 枚举：
    ...
    const FGameplayEffectSpec& Spec, ...)
{
    ...
    if (GE->DurationPolicy == EGameplayEffectDurationPolicy::Instant)
    {
        // 立即执行，不创建 ActiveGE
        ExecuteActiveEffects(Spec);
        return FActiveGameplayEffectHandle();  // 返回空 Handle
    }
    else
    {
        // Duration / Infinite：创建 ActiveGE，挂聚合器
...
**ExecuteActiveEffects**：遍历 Spec 的所有 Modifier，立即修改属性：
...
void UAbilitySystemComponent::ExecuteActiveEffects(const FGameplayEffectSpec& Spec)
{
    ...
    {
        FGameplayAttribute Attribute = ModSpec.ModifierInfo.Attribute;
        float Magnitude = ModSpec.EvaluatedMagnitude;

        // 直接改 Base 或走 Modifier 的执行逻辑
        if (ModSpec.ModifierInfo.ModifierOp == EGameplayModOp::Additive)
        {
...
**对比 Duration/Infinite**：Instant 不挂聚合器，不会参与后续的 Current 求值，直接改 Base。

#### Duration：挂聚合器、持续一段时间、定时移除

**语义**：挂聚合器、持续一段时间、定时移除。

**源码路径**：`ApplyGameplayEffectSpecToSelf` 中 Duration 的分支：
